...
All slides use Layout 13 ("Title and content 2"):
    PH 0  = Title    TITLE (1)
    PH 10 = Content  OBJECT (7)

If background is wrong, change LAYOUT to 12 (with CONTENT_PH = 1) or 14.
...
def fill_content(slide, ph_idx, paragraphs):
    """
    Fill an OBJECT placeholder with structured paragraphs.

    Strategy: set placeholder.text to activate it, then use the text frame
    to format the first paragraph and add the rest.
    """
    ...
    # Step 1: Activate the placeholder by setting .text directly
    # This forces python-pptx to create the text frame content
    ph.text = paragraphs[0].get("text", "")

    # Step 2: Now access the text frame and format the first paragraph
    tf = ph.text_frame
    tf.word_wrap = True
    p = tf.paragraphs[0]
    first = paragraphs[0]
    ...
    # Step 3: Add remaining paragraphs
...
def build_slide_1(prs, data):
    ...
    fill_content(slide, CONTENT_PH, body)
